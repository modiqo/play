...
def render_first_play_activation(human_name: str) -> str:
    return "\n".join(
        [
            ...
            "",
            "The full result above remains the result; this short guide explains the control loop around it.",
            "",
            "Here is what Hello just proved:",
            "",
            ...
            "",
            "You can see the same boundary outside the agent:",
            "",
            "```bash",
            f"rote play inspect {STARTER_PLAY_REFERENCE} --json",
            f"rote play run {STARTER_PLAY_REFERENCE}",
            "```",
            "",
            "Inspect first; the run command keeps Rote’s normal approval behavior.",
            "",
            "To make one of your own, start with `$play <do something useful>`. Play searches for an existing method first. If exploration is needed, it must start capture before the work; you can watch, correct, and steer the agent while Rote records the checked trajectory. After the result is verified, ask to save it. Work that was not captured through Rote cannot be turned into a Play afterward.",
        ]
    )
