...
class BootstrapTest(unittest.TestCase):
    ...
    def test_codex_marketplace_convergence_refreshes_removes_and_reinstalls(self) -> None:
        ...
        steps = converge_play_marketplace(
            "codex", "/bin/codex", expected_version="0.4.35", runner=runner
        )
        ...
        self.assertIn("0.4.35", steps[-1].detail)
    ...
    def test_claude_marketplace_convergence_refreshes_user_scope(self) -> None:
        ...
        steps = converge_play_marketplace(
            "claude", "/bin/claude", expected_version="0.4.35", runner=runner
        )
    ...
    @patch(
        ...
        return_value=[
            Step(
                "verify_play_plugin",
                "completed",
                "Play 0.4.35 is installed and enabled.",
                target="codex",
            )
        ],
    )
    ...
    def test_apply_existing_rote_updates_converges_installs_hooks_and_verifies(
        ...
        self.assertEqual(
            "0.4.35", _converge_marketplace.call_args.kwargs["expected_version"]
        )
