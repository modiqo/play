...
class BootstrapTest(unittest.TestCase):
    ...
    def test_local_codex_marketplace_is_not_sent_to_git_upgrade(self) -> None:
        ...
        steps = converge_play_marketplace(
            "codex", "/bin/codex", expected_version="0.4.44", runner=runner
        )
    ...
    @patch(
        ...
        return_value=[
            Step(
                "verify_play_plugin",
                "completed",
                "Play 0.4.44 is installed and enabled.",
                target="codex",
            )
        ],
    )
    ...
    def test_apply_existing_rote_updates_converges_installs_hooks_and_verifies(
        ...
        self.assertEqual(
            "0.4.44", _converge_marketplace.call_args.kwargs["expected_version"]
        )
