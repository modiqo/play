...
class BootstrapTest(unittest.TestCase):
    ...
    def test_plan_selects_top_k_and_skips_current_skill_providers(
        ...
        self.assertEqual("0.4.102", plan["play"]["target_version"])
    ...
    def test_codex_missing_marketplace_is_repaired_before_install(self) -> None:
        runner = MagicMock(side_effect=[
            ...
            MagicMock(returncode=0, stdout="installed\n", stderr=""),
            MagicMock(returncode=0, stdout=json.dumps({
                "installed": [{"pluginId": "play@play-skills", "version": "0.4.102",
                               "enabled": True}],
            }), stderr=""),
        ])

        steps = converge_play_marketplace(
            "codex", "/bin/codex", expected_version="0.4.102", runner=runner
        )
    ...
    def test_unrelated_marketplace_errors_do_not_trigger_repair(self) -> None:
        ...
        for harness, returncode, stdout, stderr in [
            ...
            with self.subTest(harness=harness, stderr=stderr, stdout=stdout):
                ...
                steps = converge_play_marketplace(
                    harness, f"/bin/{harness}", expected_version="0.4.102", runner=runner
                )
    ...
    def test_newer_marketplace_release_verifies_instead_of_rolling_back(self) -> None:
        ...
        runner = self._marketplace_runner("0.3.0", "0.4.102")
        ...
        self.assertEqual("completed", steps[-1].status)
        self.assertIn("0.4.102", steps[-1].detail)
    ...
    def test_older_marketplace_release_still_fails_verification(self) -> None:
        ...
        self.assertEqual("failed", steps[-1].status)
        self.assertIn("Expected Play 0.4.102 or newer", steps[-1].detail)

    def test_newer_healthy_plugin_is_kept_without_reinstall(self) -> None:
        runner = self._marketplace_runner("0.4.102", "0.4.102")
    ...
    def test_status_card_reports_independent_component_versions(self) -> None:
        ...
        self.assertIn("Play       0.4.74 → 0.4.102", rendered)
    ...
    @patch(
        ...
        return_value=[
            Step(
                "verify_play_plugin",
                "completed",
                "Play 0.4.102 is installed and enabled.",
                target="codex",
            )
        ],
    )
    ...
    def test_apply_existing_rote_updates_converges_installs_hooks_and_verifies(
        ...
        self.assertEqual(
            "0.4.102", _converge_marketplace.call_args.kwargs["expected_version"]
        )
