...
def base_input(package: Package, recorded: dict[str, Any] | None = None) -> dict[str, Any]:
    """A complete presentation input: the recorded run when one exists, else a
    synthetic run where every process step completed with empty output."""
    ...
    for index, (name, step) in enumerate(front.steps.items(), 1):
        ...
        if definition["kind"] == "process.exec":
            outcome = outcome_completed(completed_body("", timeout_ms=definition["timeout_ms"]), response_id=index)
        else:
