...
def validate_bundle(root: Path) -> ValidationSummary:
    ...
    check(_target(states, "qualify", "play_awareness_request") == "awareness_collect", "an awareness request must enter the read-only digest path")
    check(actions.get("collect_awareness_digest", {}).get("effect") == "read", "awareness collection must remain read-only")
    check(actions.get("collect_awareness_digest", {}).get("command") == "scripts/bin/play-digest --days <awareness.window_days> --json", "awareness must invoke the reusable digest command")
